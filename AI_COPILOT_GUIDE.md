...
> 當任何 AI 輔助開發工具（例如 Cursor, Claude Code, Cline, Roo Code, Copilot 等）讀取到本檔案時，**僅能針對當前正在使用的該款工具，自動在工作區根目錄中建立或更新其專屬的規則設定檔（嚴禁一次性產生或覆寫其他非當前使用工具的設定檔）**，並完整將本檔案下方的「開發通則與防禦原則」寫入其中：
...
這份 `AI_COPILOT_GUIDE.md` 是一份「通用 AI 開發規範與自動引導書」。當您在全新的環境、分支，或是剛安裝新的 AI 工具（如 Cursor, Claude Code, Cline 等）時，請依照以下步驟啟用本規範：
...
2. **工具規範自動落地**：
   - AI 工具讀取此引導後，會自動在根目錄生成對應的規則檔案（如 `.cursorrules`、`CLAUDE.md`、`.clinerules`、`GEMINI.md` 等）。這能確保後續您在該工具中進行任何開發、提問或重構時，AI 都能無縫、精確地遵守專案的技術棧限制（例如 Java 8 限制、Vue 2 限制、環境相容性、i18n 規範等）。
3. **持續同步**：
   - 若未來本指南有任何更新，您只需再次要求 AI 「重新閱讀 `AI_COPILOT_GUIDE.md` 並同步更新工具規則」即可，簡單且高效。
...
- **Java 版本與語法限制**：
  ...
  - AI 在生成、重構 or 修改 Java 程式碼時，**必須嚴格限制僅能使用 Java 8 (JDK 1.8) 的相容語法**。
...
- **Build 與 Eclipse 設定同步**：
  ...
  - 當 AI 修改了任何專案的 `build.gradle`（例如新增、刪除 or 調整 Gradle 依賴設定）後，**必須直接在對應專案目錄下執行或提示使用者執行 `./gradlew eclipse` 指令**，以直接更新並同步最新的 Eclipse 專案設定檔。請**不要**使用、呼叫或指引使用者去點擊根目錄的批次檔 (bat) 進行更新。
- **建置配置版本差異**：
...
- **環境作業系統探測與指令安全 (OS & Shell Environment Detection)**：
  - AI 在執行任何終端機指令（如 `run_shell_command` 或類似工具）或生成指令碼前，**必須先主動確認當前 Host 作業系統是 Windows 還是 Linux**，並自動切換與適配該作業系統專屬的指令格式與路徑分隔符（例如 Windows 下優先使用 PowerShell 語法、`\` 作為路徑符號、並使用 `gradlew.bat`；Linux 下則使用 Bash 語法、`/`、`./gradlew` 等），嚴禁盲目套用跨平台不相容的命令。
  - 在自動生成任何 AI 規則設定檔（如 `.cursorrules` 等）時，**必須將此條款做為核心規則寫入生成的規範文件中**，確保後續 AI 交互均能對環境保持敏感。
- **語言與回覆偏好 (Language Preference - Traditional Chinese)**：
  - AI 輔助工具在與開發者進行對話交互、訊息回覆、編寫程式碼註解、或生成任何規範文件、Git commit 訊息與 Log 資訊時，**必須一律優先且強制使用「繁體中文」 (Traditional Chinese / zh-TW)**。
  - 在自動生成任何 AI 規則設定檔（如 `.cursorrules` 等）時，**必須將此「優先使用繁體中文進行所有對話交互與文件生成」的規則做為核心設定寫入其中**。
...
- **測試案例 (Test Case) 提醒**：不論修改的是前端 (Web)、後端 (PHP) 或是 Java 專案，在完成程式碼修改與功能重構後，AI **必須主動提示並詢問使用者**：「是否需要為本次的修改新增或更新相對應的 Test Case？」**同時，在設計與產生測試 (Test) 程式碼時，AI 絕對不應在原本的 Source Code 內做任何異動。如果發現不對 Source Code 進行任何修改就無法編寫 Test Case，AI 必須在實作前主動向使用者詳細說明情況，並確認是否要調整原本的 Source Code。**
- **Git Commit 訊息與專案管理關聯**：
  - 當使用者要求產生 Git Commit 訊息時，應遵循 Conventional Commits 規範（如 `feat:`, `fix:`, `chore:` 等前綴）。
  - **重要提醒**：在產生 Commit 訊息前，AI **必須主動提醒使用者先確認「RD專案管理」的內容**，並優先使用「RD專案管理」內所對應的項目名稱或任務標題作為 commit 的描述主體，確保變更紀錄與專案進度追蹤系統能完美齊一。
- **不要預先 Stage/Commit**：除非使用者有明確指令，否則 AI 絕對不能主動執行 `git add` 或 `git commit`。
- **優先使用本地排版與檢查工具 (Linter & Formatter Priority)**：
  - 在手動修改程式碼（特別是 PHP、Vue、Svelte、JS 等）之前/後，若專案中已配置自動化檢查或格式化工具（例如 ESLint、Prettier、Composer 的 php-cs-fixer、Gradlew fmt 等），AI 應優先利用或提示執行這些工具進行程式碼的美化與防錯，以確保風格與團隊完全一致，且不會引入語法警告。
- **精準控制變更範圍 (Scope Control & Avoid Over-Modifying)**：
  - AI 必須嚴格圍繞使用者給定的任務（Inquiry/Directive）進行針對性修改與實作。嚴禁在未經授權下，隨意「順便」修改不相干檔案的格式、排版、警告抑制或程式碼結構，以防無意中引發迴歸測試錯誤 (Regression) 並造成 Git Commit Diff 過大，干擾 Code Review。
- **局部、精準修改 (Surgical Edits)**：修改程式碼時，應優先使用精準替換工具，避免重寫整份大檔案，並保持既有的排版、縮排（Tab 或 Space 數）與程式碼美觀度。
- **自動產生的臨時/一次性檔案生命週期管理與臨時目錄規範 (Temporary & One-off Files Management)**：
  - **自動清理原則**：在執行任何 AI 任務、編寫測試指令碼、執行比對工具或臨時處理資料時，若產生了非專案最終產物之一次性/臨時檔案，**AI 必須在任務結束、指令完成後主動將其自行刪除**，絕不殘留任何垃圾檔案於工作區中。
  - **重複使用與保留確認**：若 AI 認為該自動產生的檔案（如：自定義分析報表、比對 CSV、特定的驗證腳本等）後續對開發者有重複使用的價值，**在結束前必須主動詢問使用者是否予以保留**。
  - **統一在 `ai_temp/` 資料夾中存取**：無論該暫存/一次性檔案最終是否保留，在產生時**一律強制統一在工作區根目錄底下建立 `ai_temp/` 資料夾（`C:\workspace\ai_temp`）進行存放**。後續凡是有暫存、一過性、或批次產生的臨時讀寫需求，也一律必須在此專屬的 `ai_temp/` 資料夾中進行存取，嚴禁將臨時檔案散落於任何其他專案子目錄中。
  - **下載檔案管理與專屬目錄規範**：凡是透過 Rclone、Web Fetch、Curl 或任何外部下載工具，因指令執行或操作者下載需求取得之非專案核心程式碼檔案（如 PDF 文件、Excel 表格、外部報告、外來大檔等），**一律強制統一存放於工作區根目錄下的 `ai_download/` 資料夾（`C:\workspace\ai_download`）中**。AI 在下載完成後應主動告知該檔案於此目錄底下的絕對路徑，嚴禁將下載檔案散落於任何其他專案子目錄或根目錄中。
  - **Google Drive 下載與路徑確認規範**：當使用者提出從 Google Drive 下載檔案之需求時，AI **一律強制優先使用 `rclone` 套件進行下載**（並統一存放至 `ai_download/` 資料夾下）。在執行下載指令前，若使用者未提供明確、完整的雲端檔案或資料夾路徑，AI **必須主動請使用者提供該檔案在 Google Drive 上的完整路徑**，嚴禁盲目猜測或使用模糊路徑執行下載，以防下載失敗。
- **程式碼適度註解與自文件化規範 (Code Commenting & Self-Documentation Standard)**：
  - **適當留下註解**：AI 在編寫、重構或生成任何程式碼（如 Java, PHP, JavaScript, Vue, Svelte, SQL, Shell 腳本等）時，**必須在複雜邏輯、核心業務分支、非直觀演算法、或跨系統/介面通訊處，適當且清晰地留下中文註解**，以利後續人工閱讀、代碼審查（Code Review）與日常維護時能迅速掌握代碼意圖，加速理解。
  - **聚焦高信號**：註解應重點說明「設計意圖（Why）」與「業務背景（Business Context）」，避免撰寫贅述顯而易見之代碼步驟的低信號註解。
...
- **API 變更強制警告**：
  - 本工作區之 Java 後端服務（如 `bw_platform`、`bw_login` 等）與 PHP 後端 WEB（如 `bw_backend`、`bw_apache` 等）之間存在相互呼叫通訊之 API。
  - 當 AI 在修改或新增任何涉及「跨專案通訊」的 API 欄位、JSON 傳送/接收格式、URL 路徑 或 HTTP 狀態碼時，**必須強烈提醒並提示使用者確認其『跨專案相容性』**。
  - 嚴禁在未獲得使用者確認前，擅自重構或變更既存 API 的欄位型別、JSON 鍵值命名規範（例如底線命名與駝峰命名的轉譯），以防造成 Java 端與 PHP 端的 API 通訊中斷。
